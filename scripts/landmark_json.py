...
for i in range(len(img_paths)):
    ...
    for img_name in sorted(glob.glob(os.path.join(f, '*.jpg'))):
        ...
        if len(dets) > 0:
            ...
            save_name = os.path.join(save_path, os.path.basename(img_name)[:-4] + '.json')
            #print(points)
            #print(points[0,1])
            #m=[[points[ii, 0] , points[ii, 1] ] for ii in range(68) ]
            m=[[int( points[ii, 0]) , int(points[ii, 1]) ] for ii in range(68) ]
            k=json.dumps(m).replace('"', '')
            #print(k)
            with open(save_name, 'w', encoding='utf-8') as f:
                f.write(k)
            #np.savetxt(save_name, points, fmt='%d', delimiter=',')
            #array = [ [ b ] for x in range(68) ]
            #json.dump(array, save_name)
